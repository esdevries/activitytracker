...
test('has example data', async ({ page }) => {
  ...
  await expect(page.locator('#q-app')).toContainText(
    'A fun run competition against top athletes (Bear, panther, puma)',
  )
})
...
test('can edit activity (persists after refresh)', async ({ page }) => {
  ...
  await expect(page.getByRole('paragraph')).toContainText(
    'Going for a swim in the middle of the pacific ocean.',
  )
  ...
  await page
    ...
    .fill('Going for a ride to the mount everest, Should be safe and fine')
  ...
  await expect(page.locator('#q-app')).toContainText(
    'Going for a ride to the mount everest, Should be safe and fine',
  )
...
test('can delete an activity (persists after refresh)', async ({ page }) => {
  ...
  await expect(page.locator('#q-app')).not.toContainText(
    'A fun run competition against top athletes (Bear, panther, puma)',
  )
})
