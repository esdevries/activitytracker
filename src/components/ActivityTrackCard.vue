<template>
  <q-card class="q-mx-md q-my-md flex row">
    <q-toolbar class="q-px-md">
      ...
      <q-btn push elevated :color="activity.isTracked ? 'red' : 'green'" @click="toggleIsTracked"
        :label="activity.isTracked ? 'Untrack' : 'Track'" />
      <ToolTip anchor="bottom right" :message="`Click here to ${activity.isTracked ? 'Untrack' : 'Track'}!`" :offset="[-100, 0]"  />
    </q-toolbar>
  </q-card>
</template>
